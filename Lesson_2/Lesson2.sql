...
;
update _cities set region_id = 5468689 where region_id is null and id>0;
ALTER TABLE `geodata`.`_cities` 
...
ADD PRIMARY KEY (`id`),
ADD INDEX `Index` (`title` ASC) VISIBLE,
ADD INDEX `fk_countries_id_idx` (`country_id` ASC) VISIBLE;
;
ALTER TABLE `geodata`.`_regions` 
...
ADD CONSTRAINT `fk_cities_region_id`
  ...
  ON DELETE RESTRICT
  ON UPDATE RESTRICT;
UPDATE `geodata`.`_regions` SET `country_id` = '236' WHERE (`id` = '5468689');
ALTER TABLE `geodata`.`_regions` 
